...
class TypstSnippet {
  ...
  static preloadFonts(userFonts) {
    return {
      key: "access-model",
      forRoles: ["compiler"],
      provides: [(0, import_options_init.loadFonts)(userFonts)]
    };
  }
  /**
  ...
  async setFonts(fontInfos) {
    ...
    for (const font of fontInfos) {
      await fb.addLazyFont(font, "blob" in font ? font.blob : (0, import_init.loadFontSync)(font), font);
    }
    const compiler = await this.getCompiler();
  ...
  async pdf(o) {
    const opts = await this.getCompileOptions(o);
    opts.format = import_compiler.CompileFormatEnum.pdf;
    const compiler = await this.getCompilerReset();
    return compiler.compile(opts).then((res) => res.result).finally(() => this.removeTmp(opts));
  }
  /**
